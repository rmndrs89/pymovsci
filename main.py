...
def load_data(filepath: str | Path) -> pd.DataFrame:
    ...
    # Get the marker data
    markers_df = pd.read_csv(filepath, sep="\t", header=0)
    markers_df = markers_df.loc[
        :, [col for col in markers_df.columns if not col.endswith("_n/a")]
    ]
...
def main() -> None:
    ...
    # Load the data
    markers_df = load_data(
        DATASET_PATH
        / f"sub-{SUB_ID}"
        / "motion"
        / f"sub-{SUB_ID}_task-{TASK_FILE_NAME}_tracksys-omc_motion.tsv"
    )
    static_markers_df = load_data(
        DATASET_PATH
        / f"sub-{SUB_ID}"
        / "motion"
        / f"sub-{SUB_ID}_task-{CALIBRATION_FILE_NAME}_tracksys-omc_motion.tsv"
    )
    ...
    hjc = estimate_hip_joint_center(
        l_asis=static_markers_df[
            ["l_asis_POS_x", "l_asis_POS_y", "l_asis_POS_z"]
        ].values,
        r_asis=static_markers_df[
            ["r_asis_POS_x", "r_asis_POS_y", "r_asis_POS_z"]
        ].values,
        l_psis=static_markers_df[
            ["l_psis_POS_x", "l_psis_POS_y", "l_psis_POS_z"]
        ].values,
        r_psis=static_markers_df[
            ["r_psis_POS_x", "r_psis_POS_y", "r_psis_POS_z"]
        ].values,
    )
    ...
    for lr in "lr":
        ax.plot(
            (
                static_markers_df[f"{lr}_ank_POS_x"]
                + static_markers_df[f"{lr}_st_mm_POS_x"]
            )
            / 2,
            (
                static_markers_df[f"{lr}_ank_POS_y"]
                + static_markers_df[f"{lr}_st_mm_POS_y"]
            )
            / 2,
            (
                static_markers_df[f"{lr}_ank_POS_z"]
                + static_markers_df[f"{lr}_st_mm_POS_z"]
            )
            / 2,
            ls="none",
            marker="x",
            label=f"{lr} ankle joint",
        )
        ax.plot(
            (
                static_markers_df[f"{lr}_st_fem_POS_x"]
                + static_markers_df[f"{lr}_st_fel_POS_x"]
            )
            / 2,
            (
                static_markers_df[f"{lr}_st_fem_POS_y"]
                + static_markers_df[f"{lr}_st_fel_POS_y"]
            )
            / 2,
            (
                static_markers_df[f"{lr}_st_fem_POS_z"]
                + static_markers_df[f"{lr}_st_fel_POS_z"]
            )
            / 2,
            ls="none",
            marker="x",
            label=f"{lr} knee joint",
        )
        ax.plot(
            hjc[f"{lr}_hjc"][0],
            hjc[f"{lr}_hjc"][1],
